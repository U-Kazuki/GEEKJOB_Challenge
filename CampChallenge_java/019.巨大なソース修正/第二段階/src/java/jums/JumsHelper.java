...
public class JumsHelper {
    ...
    // 入力フォームの未記入時のエラー文は此処で生成されているのでしょうか？
    
    // chkinput（chkListにArrayListクラスとして文字列型の値を入れる）chkinputメソッド
    public String chkinput(ArrayList<String> chkList){
        String output = "";
        // 文字列型のval変数をchkListに入った値で回す所(for文の最新型)
        for(String val : chkList){// for(String val = 0; val.length; val++); 
                if(val.equals("name")){
                    output += "名前";// output = output + "名前"
                }
                if(val.equals("year")){
                    output +="年";
                }
